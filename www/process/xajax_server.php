...
function ajoutProjet($origine = null)
{
    ...
    $projet = new Projet();
    // si droit limitï¿½, on ne permet pas le choix du createur
    ...
    $defautStatus = $status2->status_id;
    ...
    $objResponse->addScript('jQuery("#myModal").modal()');
    // On n'affiche le color picker uniquement si il n'y a aucune couleur personnalisï¿½e
...
function modifProjet($projet_id = null, $origine = null)
{
    ...
    $objResponse->addScript("initselect2('$lang','" . $smarty->getConfigVars('choix_option') . "')");
    $objResponse->addScript('jQuery("#myModal").modal()');
    // On n'affiche le color picker uniquement si il n'y a aucune couleurs personnalisï¿½es
    if ($projet->couleur != '') {
...
function submitFormProjet($projet_id, $origine, $new_projet_id, $nom, $groupe_id, $statut, $charge, $livraison, $lien, $couleur, $createur_id, $iteration)
{
    ...
    // modification de la clï¿½ (projet_id) => update manuel
    ...
    if ($user->checkDroit('projects_manage_all')) {
        // rien ï¿½ faire sur le createur_id, pass? dans le POST
    } elseif ($user->checkDroit('projects_manage_own')) {
        // si c'est un planner, on lui assigne le projet ï¿½ la creation, et on checke qu'il n'a pas tent? de le changer en modif
...
function ajoutPeriode($dateDebut = '', $ligne_id = '', $periode_id = '', $heureDebut = '')
{
    ...
    // Liste des crï¿½neaux horaires exclusif
    ...
    // si il y a un user ou projet prï¿½-choisi, on le sï¿½lectionne
    if ($ligne_id != '') {
    ...
    // si periode_id present, veut dire qu'on duplique une pï¿½riode, donc charge les donn?es
    if ($periode_id != '') {
    ...
    // hack pour textarea, pour ï¿½viter l'interpretation de caractï¿½res spï¿½ciaux
    $objResponse->addAssign('notes', 'value', $periode->notes);

    if (!$_SESSION['isMobileOrTablet']) {
        $objResponse->addScript('jQuery("#date_debut").datepicker({ showWeek: true, beforeShow: function(input, inst) {$("#ui-datepicker-div").attr("inputId",this.id)}, dateFormat: "' . CONFIG_DATE_DATEPICKER . '" });');
        $objResponse->addScript('jQuery("#date_fin").datepicker({ showWeek: true, beforeShow: function(input, inst) {$("#ui-datepicker-div").attr("inputId",this.id)},  dateFormat: "' . CONFIG_DATE_DATEPICKER . '" });');
        $objResponse->addScript('jQuery("#dateFinRepetitionJour").datepicker({ showWeek: true, beforeShow: function(input, inst) {$("#ui-datepicker-div").attr("inputId",this.id)},  dateFormat: "' . CONFIG_DATE_DATEPICKER . '" });');
        $objResponse->addScript('jQuery("#dateFinRepetitionSemaine").datepicker({ showWeek: true, beforeShow: function(input, inst) {$("#ui-datepicker-div").attr("inputId",this.id)},  dateFormat: "' . CONFIG_DATE_DATEPICKER . '" });');
        $objResponse->addScript('jQuery("#dateFinRepetitionMois").datepicker({ showWeek: true, beforeShow: function(input, inst) {$("#ui-datepicker-div").attr("inputId",this.id)},  dateFormat: "' . CONFIG_DATE_DATEPICKER . '" });');
    }
...
function modifPeriode($periode_id)
{
    ...
    $periode = new Periode();
    $sql = "SELECT p.*,u.user_groupe_id from planning_periode p, planning_user u where p.user_id=u.user_id and p.periode_id=" . val2sql($periode_id);
    ...
    // comptage du nombre de jours de la pï¿½riode
    $nbJours = 0;
    ...
    // Si audit et restoration des ses tï¿½ches on recherche l'id de la derniï¿½re modification
    ...
    // hack pour lien, pour ï¿½viter l'interpretation de caractï¿½res spï¿½ciaux
...
function checkRessourceId($newRessource_id, $currentRessource_id)
{
    ...
    if ((preg_match("/^[a-zA-Z0-9]+$/", $newRessource_id) == 0) || strlen($newRessource_id) > 20) {
...
function checkLieuId($newLieu_id, $currentLieu_id)
{
    ...
    if ((preg_match("/^[a-zA-Z0-9]+$/", $newLieu_id) == 0) || strlen($newLieu_id) > 20) {
...
function moveCasePeriode($casePeriode, $jourCible, $copie_periode = false, $scope = 'seule')
{
    ...
    // d?coupage des chaine pour recup des valeurs
    ...
    // Verification si c'est une multi-affectation
    ...
    // vï¿½rification si lors du drag'n'drop on change le user
    ...
    foreach ($listePeriodeModif as $p) {
        ...
        if ($userCible->db_load(array('user_id', '=', $userSearch))) {
            ...
            if ($user->checkDroit('tasks_modify_own_task')) {
                // si droits limitï¿½s aux taches on checke que le projet cible est autoris?
                $projTmp = new Projet();
            ...
            if ($user->checkDroit('tasks_modify_own_task')) {
                // si droits limites aux taches on checke que le projet cible est autorise
                $projTmp = new Projet();
        ...
        if ($copie_periode == 'true') {
            ...
            // Verification que le lieu est disponible
            ...
            // on envoie que si la personne assignï¿½e n'est pas la personne connect?e
            if ($copie->user_id != $user->user_id) {
                $copie->envoiNotification('creation');
            }

        } else {
			// mise a jour des infos de la pï¿½riode dï¿½placï¿½e
			if ($projetCible->isSaved()) {
				$periode->projet_id = $projetCible->projet_id;
			} elseif($userCible->isSaved()) {
				// si c'est un dï¿½placement de user, on gï¿½nï¿½re un nouveau link_id
				if ($type_move_user == true)
				{
					...
					if ($scope == "seule")
					{
						...
						// copie des fichiers joints vu que la tï¿½che est dï¿½liï¿½e des autres
			...
			if(count($chaines2) < 4 && $scope == 'seule' && !is_null($periode->date_fin) && $jourOrigine == $periode->date_fin && $jourDestination > $periode->date_debut){
				...
				// modif calcul du nombre de jour de decalage entre le debut de la periode et la case cliquï¿½e
				$nbJoursDecalOrig = 0;
				$nbJoursDecalOrig = getNbJours($periode->date_debut, $jourOrigine);

				// modif calcul du nombre de jour de la pï¿½riode pour report sur la nouvelle date
				if (!is_null($periode->date_fin)) {
					$nbJours = 0;
					$nbJours = getNbJours($periode->date_debut, $periode->date_fin);
            ...
            // modif calcul du nombre de jour de decalage entre le debut de la periode prï¿½cï¿½dente et la nouvelle
            $nbJoursDecalDest = 0;
            ...
            //$nbJoursDecalDest = 0;
            // Vï¿½rification que la ressource est disponible
            if (!is_null($periode->ressource_id)) {
            ...
            // Vï¿½rification que le lieu est disponible
            if (!is_null($periode->lieu_id)) {
            ...
            // on fait la notification ici et non dans le db_save() sinon ï¿½a va s'appliquer ? toutes les taches filles
            // on envoie que si la personne assignï¿½e n'est pas la personne connect?e
	 ...
	 // Verification si le rï¿½pertoire upload est encore nï¿½cessaire 
	...
	// chargement de la fenetre de rï¿½ussite
    $objResponse->addScript('location.reload();');
...
function modifUser($user_id = null)
{
    ...
    if ($user->checkDroit('users_manage_team')) {
		if(is_null($user->user_groupe_id)){
            $objResponse->addAlert('You are not linked to a team');
            return $objResponse->getXML();
		}
...
function submitFormUser($user_id, $user_id_origine, $user_groupe_id, $nom, $email, $login, $password, $visible_planningOui, $couleur, $notificationsOui, $envoiMailPwd, $droits, $adresse, $telephone, $mobile, $metier, $commentaire, $login_actifOui, $specific_users_ids, $google_2fa_reset)
{
    ...
    // on checke que le user_id n'existe pas dï¿½jï¿½
    if ($user_id_origine == '') {
        //si cr?ation de user
        $userTest = new USer();
        ...
        if (trim($login) != '' && $userTest->db_load(array('login', '=', trim($login)))) {
            $objResponse->addAlert($smarty->getConfigVars('login_existant'));
            return $objResponse;
        }
    } else {
        // si user existant on vï¿½rifie que les champs ne vont pas ?craser un existant (login et identifiant)
        $userTest = new USer();
    ...
    if ($user_id_origine == '') {
        // on met ï¿½ jour le user_id uniquement ï¿½ la creation pour ï¿½viter l'ï¿½crasement par un petit rus?
	...
	if($google_2fa_reset == 'true'){
		$user_form->google_2fa = 'setup';
	}
...
function submitFormProfil($user_id, $email, $password, $dateformat, $notificationsOui, $positionPlanning, $vueDefautPlanning, $vueDefautPersonne, $vueDefautMois, $vueDefautLarge)
{
    ...
    // Creation du tableau json pour stockage de prï¿½fï¿½rences
    $user->preferences = json_encode($preferences);
    if (!$user->db_save()) {
        $objResponse->addAlert(addslashes($smarty->getConfigVars('changeNotOK')));
        return $objResponse;
    }
    // Si on est dans le cas d'un changement de format de date, on efface les anciennes donnï¿½es de la session
...
function submitFormPeriode($periode_id, $projet_id, $user_id, $date_debut, $conserver_duree, $date_fin, $nb_jours, $duree, $heure_debut, $heure_fin, $matin, $apresmidi, $repetition, $dateFinRepetitionJour, $dateFinRepetitionSemaine, $dateFinRepetitionMois, $nbRepetitionJour, $nbRepetitionSemaine, $nbRepetitionMois, $jourSemaine, $exceptionRepetition, $appliquerATous, $statut_tache, $lieu, $ressource, $livrable, $titre, $notes, $lien, $custom, $fichiers, $link_id, $notif_email, $updateoccurrences = 'true')
{
    ...
    if ($conserver_duree === 'true') {
        // on reprend la durï¿½e existante (seulement en modif de pï¿½riode)

        // on charge la pï¿½riode de la BD pour rï¿½cupï¿½rer les anciennes date, pour calculer nb de jour
        ...
        // affichï¿½ seulement en crï¿½ation
        ...
        while ($i <= $nbJours) {
            ...
            if (in_array($dateFin->format('w'), explode(',', CONFIG_DAYS_INCLUDED)) && !array_key_exists($dateFin->format('Y-m-d'), $joursFeries)) {
                $i++;
            }
        }
        ...
        // pas de date de fin renseignï¿½e, on gï¿½re la durï¿½e

        // si aucune info renseignï¿½e, on met la journï¿½e entiï¿½re pour la tache
	...
	// on construit un tableau indiquant quoi faire pour chaque user assignï¿½
    ...
    foreach ($user_id as $current_user_id) {
		if(in_array($current_user_id, $usersExistants)){
			...
			$usersTraitement[$current_user_id] = 'creation';
		}
	}
    ...
    foreach ($usersTraitement as $current_user_id => $todo) {
		if($todo == 'delete'){
			continue;
		}
        $periodeTmp = new Periode();
		if($todo == 'update'){
			if(!$periodeTmp->db_load(array('user_id', '=', $current_user_id, 'link_id', '=', $periode->link_id, 'date_debut', '=', $periodeSave->date_debut))){
				echo 'Task not found - ' . $current_user_id . ' - '. $periode->link_id;die;
			}
		}
		$dataTmp = $periode->getData();
		$dataTmp['periode_id'] = $periodeTmp->periode_id;
        $periodeTmp->setData($dataTmp);
		if($todo == 'creation'){
			$periodeTmp->periode_id = 0;
		}
		$periodeTmp->user_id = $current_user_id;

        if ($periodeTmp->check() !== true) {
        ...
        $projet = new Projet();
        $projet->db_load(array('projet_id', '=', $periodeTmp->projet_id));
        if ($user->checkDroit('tasks_modify_own_project') && $projet->createur_id != $user->user_id) {
            $objResponse->addAlert(addslashes($smarty->getConfigVars('ajax_droitsInsuffisants')));
            $objResponse->addScript('location.reload();');
            return $objResponse;
        ...
        if ($user->checkDroit('tasks_modify_own_task') && $projet->createur_id != $user->user_id && $periodeTmp->user_id != $user->user_id) {
            $objResponse->addScript("document.getElementById('butSubmitPeriode').disabled=false;");
            $objResponse->addScript("document.getElementById('divPatienter').style.display='none';");
            $objResponse->addAlert(addslashes($smarty->getConfigVars('ajax_droitsInsuffisants')));
            return $objResponse;
        ...
        if ($user->checkDroit('tasks_view_only_own') && $periodeTmp->user_id != $user->user_id) {
            $objResponse->addScript("document.getElementById('butSubmitPeriode').disabled=false;");
            $objResponse->addScript("document.getElementById('divPatienter').style.display='none';");
            $objResponse->addAlert(addslashes($smarty->getConfigVars('ajax_droitsInsuffisants')));
            return $objResponse;
    ...
    foreach ($usersTraitement as $current_user_id => $todo) {
        $periodeTmp = new Periode();
		if($todo == 'delete'){
			if(!$periodeTmp->db_load(array('user_id', '=', $current_user_id, 'link_id', '=', $periode->link_id))){
				echo 'Task not found - ' . $current_user_id . ' - '. $periode->link_id;die;
			}
			$periodeTmp->db_delete();
			continue;
		}
		if($todo == 'update'){
			if(!$periodeTmp->db_load(array('user_id', '=', $current_user_id, 'link_id', '=', $periode->link_id))){
        ...
        if (CONFIG_PLANNING_ONE_ASSIGNMENT_MAX_PER_DAY == 1) {
            //on checke qu'il n'y ait aucun jour en commun entre cette tache et les autres taches du meme user
            $sql = "SELECT * FROM planning_periode ";
            if (!is_null($periodeTmp->date_fin)) {
                $sql .= " WHERE ((date_debut >= " . val2sql($periodeTmp->date_debut) . "   AND date_debut <= " . val2sql($periodeTmp->date_fin) . ")";
                $sql .= " OR (date_fin IS NOT NULL AND date_fin >= " . val2sql($periodeTmp->date_debut) . " AND date_fin <= " . val2sql($periodeTmp->date_fin) . ")";
            } else {
                $sql .= " WHERE ((date_fin IS NOT NULL AND date_debut <= " . val2sql($periodeTmp->date_debut) . " AND  date_fin >= " . val2sql($periodeTmp->date_debut) . ")";
                $sql .= " OR (date_fin IS NULL AND date_debut = " . val2sql($periodeTmp->date_debut) . ")";
            }
            $sql .= " )     AND user_id = " . val2sql($periodeTmp->user_id);
            if ($periodeTmp->isSaved()) {
                $sql .= ' AND periode_id <> ' . val2sql($periodeTmp->periode_id);
            }
            $periodesTest = new GCollection('Periode');
            $periodesTest->db_loadSQL($sql);
            if ($periodesTest->getCount() > 0) {
                $periodeTmp2 = $periodesTest->fetch();
                $projetTmp = new Projet();
                $projetTmp->db_load(array('projet_id', '=', $periodeTmp2->projet_id));
                $objResponse->addScript("document.getElementById('butSubmitPeriode').disabled=false;");
                $objResponse->addScript("document.getElementById('divPatienter').style.display='none';");
                $objResponse->addAlert(addslashes(sprintf($smarty->getConfigVars('ajax_jourDejaOccupe'), $projetTmp->nom, $periodeTmp->date_debut, $periodeTmp->date_fin)));
                return $objResponse->getXML();
        ...
        // on fait la notification ici et non dans le db_save() sinon ï¿½a va s'appliquer ï¿½ toutes les taches filles
        // on envoie que si la personne assignï¿½e n'est pas la personne connectï¿½e
        if ($notif_email == 'true' && $periodeTmp->user_id != $user->user_id) {
            if ($creationPeriode) {
                $periodeTmp->envoiNotification('creation', $repetition);
            } else {
                $periodeTmp->envoiNotification('modification', $repetition);
            }
        }
        ...
        if ($repetition != '' && $repetition != 'undefined') {
            $periodeTmp->parent_id = $periodeTmp->periode_id;
            $periodeTmp->db_save();
            switch ($repetition) {
                case 'jour':
            ...
            // Controle que la date de fin de rï¿½pï¿½tition est supï¿½rieure ï¿½ la date de d?but
            ...
            // Controle que la date de fin de rï¿½pï¿½tition est au moins 7 jours aprï¿½s celle du dï¿½but si on choisit semaine
            ...
            // Controle que la date de fin de rï¿½pï¿½tition est au moins 30 jours aprï¿½s celle du dï¿½but si on choisit mois
            ...
            // Si la rï¿½pï¿½tition ne fonctionne pas...
            $reponse_repetition = $periodeTmp->repeter($repetition, userdate2sqldate($dateFinRepetition), $periodeTmp->duree_details, $nbRepetition, $jourSemaine, $exceptionRepetition);
    ...
    if (!isset($creationPeriode)) {
        if ($appliquerATous === 'true') {
            ...
            // Mise ï¿½ jour de toutes les occurrences
...
function supprimerPeriode($periode_id, $fullscope = 'true', $notif_email = 'false')
{
    ...
    // Suppression des fichiers joints s'ils ne sont plus utilisï¿½s
...
function submitFormStatus($status_id, $new_status_id, $nom, $commentaire, $affichage, $barre, $gras, $italique, $souligne, $defaut, $affichage_liste, $pourcentage, $couleur, $priorite)
{
    ...
    // Prï¿½fï¿½rence de sï¿½lection par dï¿½faut
...
function purgerAudit()
{
    ...
    // Purge de l'audit avec la rï¿½tention prï¿½vue
...
function google_2fa_check_code($code)
{
    $objResponse = new xajaxResponse();
    $smarty = new MySmarty();
	...
	$secret = trim($user->cle);
	if ($g->checkCode($secret, $code)) {
		$user->google_2fa = 'ok';
		$user->db_save();
		$_SESSION['2fa_ok'] = 'ok';
	} else{
        $objResponse->addAlert(addslashes($smarty->getConfigVars('google_2fa_code_failed')));
        return $objResponse;
	}

    $objResponse->addRedirect($user->vueDefaut());
    return $objResponse;
}
